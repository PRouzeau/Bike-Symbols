...
module bike (diam=380, ht=35, ana=2, fill=false) {
  ...
  dmirrory() 
    diff() {
      cylz(dw,ht, 0,wbase2,2);
      if (!fill) {
        cylz(diw,ht+20, 0,wbase2,-10);    cubez(diam,diam*0.05,ht+20, -diam/2,diam/1.4,-10);
        t(0,wbase2)  
          dmirrory()   
          rotz (60)
            cubez(dw,diam*0.05,ht+20, diam/2,0,-10); 
      }
    }  
  t (0,0) {  
    // leg
 ...
 scale ([ana,1,1]) {  
  dmirrory() 
    diff() { // wheels
      cylz(dw,ht, 0,wbase2);
      if (!fill) {
        cylz(diw,ht+10, 0,wbase2,-2);
        if (!link) {
          cubez(dw,diam*0.05,ht+20, -diam/2,wbase2,-10);
        t(0,dw/1.4)  
          dmirrory()   
          rotz (60)
            cubez(dw,diam*0.05,ht+20, diam/2,0,-10); 
  ...
  hull() { // top arm
    cylz (d6,ht, -diam*1.10, -diam*0.18);
